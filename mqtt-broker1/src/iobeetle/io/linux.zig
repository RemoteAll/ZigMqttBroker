...
pub const IO = struct {
    ...
    fn flush(self: *IO, wait_nr: u32, timeouts: *usize, etime: *bool) !void {
        ...
        while (self.completed.pop()) |completion| {
            // Zig 0.15.2 兼容性修�? kernel_timespec 字段名从 tv_sec/tv_nsec 改为 sec/nsec
    ...
    ///
    /// TODO(Linux):
    /// - Linux kernel �?.19 supports the IORING_ASYNC_CANCEL_ALL and IORING_ASYNC_CANCEL_ANY flags,
    ...
    ///   cancellation stage.
    /// - Linux kernel �?.0 supports `io_uring_register_sync_cancel` which would remove the `queued`
    ///   cancellation stage.
                        ...
                        if (completion.result < 0) {
                            break :result switch (@as(posix.E, @enumFromInt(-completion.result))) {
                                ...
                                .INVAL => unreachable,
                                    ...
                                    std.log.err("cancel errno: {any}", .{errno});
                                    break :result error.Unexpected;
                                },
                            };
                        }
                    };
                        ...
                        if (completion.result < 0) {
                            const err = switch (@as(posix.E, @enumFromInt(-completion.result))) {
                                ...
                                .PROTO => error.ProtocolFailure,
                                    ...
                                    std.log.err("accept errno: {any}", .{errno});
                        ...
                        if (completion.result < 0) {
                            const err = switch (@as(posix.E, @enumFromInt(-completion.result))) {
                                ...
                                .NOSPC => error.NoSpaceLeft,
                                    ...
                                    std.log.err("close errno: {any}", .{errno});
                                    break :blk error.Unexpected;
                                },
                            };
                            break :blk err;
                        ...
                        if (completion.result < 0) {
                            const err = switch (@as(posix.E, @enumFromInt(-completion.result))) {
                                ...
                                .TIMEDOUT => error.ConnectionTimedOut,
                                    ...
                                    std.log.err("connect errno: {any}", .{errno});
                                    break :blk error.Unexpected;
                                },
                            };
                            break :blk err;
                        ...
                        if (completion.result < 0) {
                            const err = switch (@as(posix.E, @enumFromInt(-completion.result))) {
                                ...
                                .INVAL => unreachable,
                                    ...
                                    std.log.err("fsync, errno: {any}", .{errno});
                                    break :blk error.Unexpected;
                                },
                            };
                            break :blk err;
                        ...
                        if (completion.result < 0) {
                            const err = switch (@as(posix.E, @enumFromInt(-completion.result))) {
                                ...
                                .TXTBSY => error.FileBusy,
                                    ...
                                    std.log.err("openat errno: {any}", .{errno});
                                    break :blk error.Unexpected;
                                },
                            };
                            break :blk err;
                        ...
                        if (completion.result < 0) {
                            const err = switch (@as(posix.E, @enumFromInt(-completion.result))) {
                                ...
                                .NOTDIR => error.NotDir,
                                    ...
                                    std.log.err("statx errno: {any}", .{errno});
                                    break :blk error.Unexpected;
                                },
                            };
                            break :blk err;
                ...
                .timeout => {
                    ...
                    const err = switch (@as(posix.E, @enumFromInt(-completion.result))) {
                        ...
                        .TIME => {}, // A success.
                            ...
                            std.log.err("timeout errno: {any}", .{errno});
                            break :blk error.Unexpected;
                        },
                    };
        ...
        switch (kind) {
            ...
            .block_device => {
                ...
                switch (os.linux.E.init(os.linux.ioctl(
                    ...
                    else => |err| return std.log.err("open_file:ioctl {any}", .{err}),
                }
                ...
                if (method == .create or method == .create_or_open) {
                    ...
                    var read_buf: [superblock_zone_size]u8 align(SECTOR_SIZE) = undefined;
    ...
    fn fs_is_tmpfs(dir_fd: fd_t) !bool {
        var statfs: StatFs = undefined;

        while (true) {
            const res = fstatfs(dir_fd, &statfs);
            switch (os.linux.E.init(res)) {
                .SUCCESS => {
                    return statfs.f_type == TmpfsMagic;
                },
                .INTR => continue,
                else => |err| return std.log.err("fs_is_tmpfs {any}", .{err}),
            }
        }
    }
    ...
    fn fs_supports_direct_io(dir_fd: fd_t) !bool {
        ...
        _ = try std.fmt.bufPrint(&cookie, "{0x}", .{std.crypto.random.int(u64)});
        ...
        while (true) {
            ...
            switch (os.linux.E.init(res)) {
                ...
                .INTR => continue,
                .INVAL => return false,
                else => |err| return std.log.err("fs_supports_direct_io {any}", .{err}),
            }
        }
    }
    ...
    fn fs_allocate(fd: fd_t, size: u64) !void {
        ...
        while (true) {
            ...
            switch (os.linux.E.init(rc)) {
                ...
                else => |errno| return std.log.err("fs_allocate {any}", .{errno}),
            }
        }
    }
};
